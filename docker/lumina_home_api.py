...
PING_CACHE_TTL = int(os.getenv("LUMINA_PING_CACHE_TTL", "30"))
LOAD_CACHE_TTL = int(os.getenv("LUMINA_LOAD_CACHE_TTL", "25"))
SERVER_TOTALS_CACHE_TTL = int(os.getenv("LUMINA_SERVER_TOTALS_CACHE_TTL", "120"))
...
PREFETCH_INTERVAL = int(os.getenv("LUMINA_PREFETCH_INTERVAL", "15"))
PREFETCH_TOP_N = int(os.getenv("LUMINA_PREFETCH_TOP_N", "10"))
CONNECTION_POOL_SIZE = int(os.getenv("LUMINA_CONNECTION_POOL_SIZE", "10"))
GZIP_MIN_SIZE = int(os.getenv("LUMINA_GZIP_MIN_SIZE", "1024"))
...
@dataclass
class ServerTotalsCacheEntry:
    expires_at: float
    value: dict[str, Any]


class ConnectionPool:
    def __init__(self, pool_size: int = CONNECTION_POOL_SIZE) -> None:
    ...
    def return_opener(self, opener):
        try:
            self._pool.put_nowait(opener)
        except Exception:
            pass
...
class LuminaHomeService:
    def __init__(self) -> None:
        ...
        self.totals_cache: dict[str, ServerTotalsCacheEntry] = {}
    ...
    def get_server_totals(self, uuid: str) -> dict[str, Any]:
        ...
        with self.totals_lock:
            cached = self.totals_cache.get(uuid)
        ...
        for server in snapshot.get("servers", []):
            ...
            with self.totals_lock:
                self.totals_cache[uuid] = ServerTotalsCacheEntry(
                    expires_at=now + SERVER_TOTALS_CACHE_TTL,
                    value=result,
                )
            return result
        return {
...
class LuminaHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if parsed.path == "/lumina-api/home-snapshot":
                snapshot = SERVICE.get_snapshot()
                if snapshot is None:
                    self.write_json(HTTPStatus.SERVICE_UNAVAILABLE, {"success": False, "error": "snapshot not ready"})
                    return
                self.write_json(HTTPStatus.OK, {"success": True, "data": {"snapshot": snapshot}})
                return
    ...
    def write_json(self, status: HTTPStatus, payload: dict[str, Any]) -> None:
        ...
        if HAS_MSGPACK and "application/msgpack" in accept:
        ...
        use_gzip = "gzip" in accept_encoding and len(body) >= GZIP_MIN_SIZE
...
def prefetch_worker() -> None:
    while True:
        ...
        try:
            ...
            top_uuids = [str(s.get("id")) for s in servers[:PREFETCH_TOP_N] if s.get("id") is not None]
            for uuid in top_uuids:
                try:
                    SERVICE.get_service_history(uuid)
                except Exception:
                    pass
        except Exception:
            logging.exception("prefetch worker error")
