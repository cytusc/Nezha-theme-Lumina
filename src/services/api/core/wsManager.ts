...
import { getServerStreamUrl } from "../utils";
...
class WebSocketManager {
  ...
  private messageBuffer: unknown = null;
  private flushHandle: number | null = null;

  constructor(options: WebSocketConnectionOptions = {}) {
    this.connectionUrl = options.url || getServerStreamUrl();
  ...
  private readonly processBufferedMessage = () => {
    this.flushHandle = null;
    if (this.messageBuffer !== null) {
      const data = this.messageBuffer;
      ...
      this.socket.onmessage = (event) => {
        ...
        try {
          ...
          if (event.data instanceof ArrayBuffer) {
            data = decode(new Uint8Array(event.data)) as unknown;
          } else {
            data = JSON.parse(event.data) as unknown;
          }
          apiLogger.debug("WebSocket message received", {
            messageLength: event.data instanceof ArrayBuffer ? event.data.byteLength : event.data.length,
            messageCount: this.messageCount,
          ...
          if (this.flushHandle === null) {
            this.flushHandle = requestAnimationFrame(this.processBufferedMessage);
          }
        } catch (error) {
          apiLogger.error("WebSocket message parse error", { error });
        }
      };
