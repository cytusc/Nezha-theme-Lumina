...
async function validatedGet<TSchema extends z.ZodTypeAny>(
  ...
  try {
    ...
    apiLogger.error("Nezha API Error", { path }, error instanceof Error ? error : new Error(String(error)));
    throw error;
  }
}
...
class NezhaApiAdapter {
  ...
  async getServerServices(serverId: number, period: string): Promise<NezhaServiceInfo[]> {
    ...
    if (cached && cached.expiresAt > now) {
      apiLogger.debug("Cache Hit - Server Services", { serverId, period });
      return cached.data;
    }

    const data = await validatedGet(
  ...
  async getHomeBootstrap(): Promise<HomeBootstrapPayload> {
    return await validatedGet("/lumina-api/home-bootstrap", HomeBootstrapSchema);
  }
  ...
  async getHomepagePingOverviewBatch(
    ...
    return await validatedGet(`/lumina-api/ping-overview?${search.toString()}`, PingOverviewMapSchema, options);
  }
  ...
  rememberNodeDisplay(node: NodeDisplay) {
  ...
  mapStreamServerToNodeDisplay(server: NezhaStreamServer, nowMs = Date.now()): NodeDisplay {
