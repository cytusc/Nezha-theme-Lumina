...
import { requestDeduplicator } from "@/services/api/core/requestDeduplicator";

const DEFAULT_PING_REFRESH_INTERVAL = 30_000;
const PING_OVERVIEW_MISSING_GRACE_ROUNDS = 1;
const ADAPTIVE_MIN_INTERVAL = 5_000;
const ADAPTIVE_MAX_INTERVAL = 120_000;
const ADAPTIVE_INCREASE_FACTOR = 1.5;
const ADAPTIVE_DECREASE_FACTOR = 0.5;
const ADAPTIVE_STABILITY_THRESHOLD = 3;
...
let adaptiveInterval = DEFAULT_PING_REFRESH_INTERVAL;
let stableRounds = 0;
let isPageVisible = typeof document !== "undefined" ? document.visibilityState !== "hidden" : true;
...
function schedulePingRefresh(intervalMs?: number) {
  ...
  const resolvedInterval = intervalMs ?? adaptiveInterval;
...
function updateAdaptiveInterval(touchedCount: number) {
  if (touchedCount > 0) {
    stableRounds = 0;
    adaptiveInterval = Math.max(
      ADAPTIVE_MIN_INTERVAL,
      adaptiveInterval * ADAPTIVE_DECREASE_FACTOR,
    );
  } else {
    stableRounds++;
    if (stableRounds >= ADAPTIVE_STABILITY_THRESHOLD) {
      adaptiveInterval = Math.min(
        ADAPTIVE_MAX_INTERVAL,
        adaptiveInterval * ADAPTIVE_INCREASE_FACTOR,
      );
      stableRounds = 0;
    }
  }
...
if (typeof document !== "undefined") {
  document.addEventListener("visibilitychange", () => {
    ...
    if (isPageVisible && wasHidden && !pingRefreshInFlight && scheduledVisibleUuids.length > 0) {
      void refreshPingOverview();
    }
  });
}
...
if (typeof window !== "undefined") {
  window.addEventListener("online", () => {
    isOnline = true;
    if (!pingRefreshInFlight && scheduledVisibleUuids.length > 0) {
      void refreshPingOverview();
    }
  });
...
function commitPingOverview(visibleKey: string, items: Map<string, PingOverviewItem>) {
  ...
  updateAdaptiveInterval(touched.size);
...
async function buildOverviewMap(clientUuids: string[]) {
  ...
  const dedupeKey = `ping-overview:${normalizedUuids.join("|")}`;

  const items = await requestDeduplicator.deduplicate(
    dedupeKey,
    () => getHomepagePingOverviewBatch(normalizedUuids, {
...
async function refreshPingOverview() {
  ...
  // On first call, wait briefly for bootstrap seed to arrive
  if (!seeded) {
...
export function ensurePingOverviewStarted(visibleUuids: string[]) {
  ...
  if (scheduledVisibleKey !== visibleKey) {
    ...
    // Bootstrap seed already provided the full ping dataset from the server.
    // No need to immediately re-fetch — just schedule the next periodic refresh.
    if (seeded) {
      schedulePingRefresh();
      return;
    }
