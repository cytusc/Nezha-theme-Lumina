...
import { useHomepagePingRegistration, usePingMini, usePingMiniBuckets } from "@/hooks/usePingMini";
import {
  formatBytes,
  formatExpireDays,
  formatTrafficRate,
...
import { prefetchLoadRecords } from "@/hooks/useRecords";
...
export const NodeCard = memo(function NodeCard({
  ...
  useHomepagePingRegistration(uuid, true);
  ...
  const formatted = useMemo(() => {
    ...
    return {
      footerTags, expire, uptime, subtitle,
      loadFraction, upRate, downRate,
      ramDetail, swapDetail, diskDetail,
    };
  }, [
    node?.tags, node?.group, node?.public_remark,
    node?.os, node?.arch, node?.virtualization,
    node?.expired_at, node?.uptime, node?.cpu_cores,
    node?.load1, node?.netUp, node?.netDown,
    node?.ramUsed, node?.ramTotal,
    node?.swapUsed, node?.swapTotal,
  ...
  const { footerTags, expire, uptime, subtitle,
          loadFraction, upRate, downRate,
          ramDetail, swapDetail, diskDetail } = formatted;
      ...
      <div className="server-card-content">
        <header className="server-card-header">
          ...
          <Link
            ...
            title="查看详情"
            onMouseEnter={() => prefetchLoadRecords(node.uuid, 24)}
          >
              ...
              <div className="server-health-chart-wrap">
                {hasHomepagePingBinding ? (
                  ...
                  <div className="server-health-placeholder">
                    {hasHomepagePingBinding ? "加载中..." : "未配置首页 Ping"}
                  </div>
                )}
